...
for folder in os.listdir('./data/json/'):
    ...
    for file in os.listdir('./data/json/' + folder):
        g_data = []
        file_dir = './data/images/' + folder + '/' + os.path.splitext(file)[0] + '.jpg'
        json_dir = './data/json/' + folder + '/' + file
        
        img = Image.open(file_dir)
        w, h = img.size
        

        with open('./data/json/' + folder + '/' + file) as f:
          data = json.load(f)
          ...
          for i, val in enumerate(bbox):
              ...
              # if bbox_id_cls == "tick_grouping":
              #     print("mark_label_file", file_dir)
              #     print("mark_label_json",json_dir)
              # elif bbox_id_cls == "value_label":
              #     print("value_label", file_dir)
              #     print("value_label_json",json_dir)
              
              if bbox_cls_index == 2:
                  bbox_cls_oh = 1
              elif bbox_cls_index == 0:
                  bbox_cls_oh = 2
              elif bbox_cls_index == 1:
                 bbox_cls_oh = 3
              else:
                  bbox_cls_oh = 0
          ...
          with open('./data/csv/' + folder + '/' + os.path.splitext(file)[0] + '.csv', 'w+') as csvf:
               ...
               header = ["id", "chart_1", "chart_2", "chart_3", "chart_4", "chart_5", "chart_6", "chart_7", "chart_8", "chart_9", "chart_10", "chart_11", "chart_12", "chart_13", "chart_14", "chart_15", "width", "height", "x0", "x1", "x2", "x3", "y0", "y1", "y2", "y3", "class"]
